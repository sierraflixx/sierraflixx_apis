...
class Helper
{
    ...
    public function sendResponse($message, array $data, $code): void
    {
    ...
    public function populateObject(Model $object, array $data, bool $attach_id = false, array $excepts = []): Model
    {
        if ($attach_id) {
            ...
            $id = "id";
            $object->$id = bin2hex(random_bytes(32));
        }
        foreach ($data as $key => $value) {
    ...
    public function setIfContained(string $key, array $source, $alternative, string $format = '')
    {
        $data = $alternative;
